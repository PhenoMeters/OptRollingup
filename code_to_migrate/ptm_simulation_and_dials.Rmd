...
```{r}
set.seed(25)
sequence = generate_random_protein(20)
print(sequence)
```
...
We add specific identifiers to the sequence to indicate a modification site. Here "@" is the identifier.

```{r}
modified_sequence = add_ptms(sequence, amino_acid, identifier)
print(modified_sequence)
```

So modification sites are located at the following sequence indices:

```{r}
print(get_ptm_locations(sequence, amino_acid))
```

Provided a synthetic trypsin digestion via OrgMassSpecR, we create a peptide-site mapping:
```{r}
digest_results = OrgMassSpecR::Digest(sequence)
ptm_site_mapping = get_ptm_site_mapping(sequence, digest_results, amino_acid, drop_na = TRUE)
ptm_site_mapping %>% knitr::kable()
```

We then can iteratively combine adjacent peptides from the complete digestion to simulate an imperfect digestion:

```{r}
#Add peptide effect before merging peptides (effects need to be combined when peptides are merged)
  #Not sure if this actually makes sense to do before merging. This effectively reduces the effect variance for each peptide which, post merge, has multiple tryptic digestion sites
# ptm_site_mapping = add_peptide_effect(ptm_site_mapping)


ptm_site_mapping_imperfect = imperfect_digest(ptm_site_mapping, 0)
ptm_site_mapping_imperfect %>% knitr::kable()
```

And from this imperfect digestion, we can simulate modifications for each site, and map them to peptide counts:

```{r}
modify_indices = get_modify_indices(ptm_site_mapping, 10)
peptide_counts = get_peptide_counts(ptm_site_mapping_imperfect, modify_indices)
peptide_counts %>% knitr::kable()
```
...
```{r}
 peptide_counts %>%
        group_by(
          ...
          site_pattern) %>%
        summarise(
          #site_pattern = paste(site, collapse = ";"), 
          true_count = sum(peptide_count)
          ) %>%
        select(-peptide) %>%
